...
export default function TriviaGame(props: TriviaGameProps) {
    ...
    const [lastCollectibleTime, setLastCollectibleTime] = useState<number>(0);
  ...
  useEffect(() => {
    if (showCollectibleModal) {
      ...
      return () => clearTimeout(timer); // Clean up timer if component unmounts or modal closes early
    }
...
useEffect(() => {
  ...
  const handleTimeUp = () => {
    ...
    if (activePowerUp === 'shield') {
      ...
      // ⚡ Use a micro delay to continue next question properly
      setTimeout(() => {
        handleNextQuestion();
      }, 500);
    ...
    setSession(prev => {
      ...
      if (newHearts <= 0) {
        // If no hearts left, game over after time is up
        setTimeout(() => {
          setGameState('gameOver');
        }, 500);
      } else {
        // Show the "Next" button if time is up and a heart is lost
...
if (isMaintenance) {
    return (
      <AppLayout breadcrumbs={breadcrumbs}>
        <Head title="StudyQuest - Maintenance" />
        <div className="min-h-screen flex items-center justify-center text-center px-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold mb-4 text-primary">🚧 Under Maintenance</h1>
            <p className="text-lg text-muted-foreground">StudyQuest is currently undergoing improvements.<br />Please check back later!</p>
          </div>
        </div>
      </AppLayout>
    );
