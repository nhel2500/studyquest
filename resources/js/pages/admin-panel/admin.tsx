...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState('questionManager'); // State to manage the active tab
    ...
    axios.get('/admin/fetch-leaderboard')
      .then(response => {
        const uniqueUsers = Array.from(
            new Map(response.data.map((user: any) => [user.id, user])).values()
          );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] text-white flex font-sans">
      ...
      <div className="w-64 bg-[#0f0c29] flex flex-col items-center py-10 border-r border-purple-900 shadow-lg">
        <img src="/official_logo.png" alt="StudyQuest" className="w-60 mb-4" />
      ...
      <div className="flex-1 p-10">
        ...
        <div className="flex justify-between items-start mb-10">
          <div>
            ...
            <nav className="text-purple-400 space-x-6">
  <button
    className={`hover:text-white transition-colors duration-200 mt-12 ${activeTab === 'questionManager' ? 'border-b-2 border-purple-500' : ''}`}
    onClick={() => setActiveTab('questionManager')}
  >
    Question Manager
  </button>
  <button
    className={`hover:text-white transition-colors duration-200 ${activeTab === 'database' ? 'border-b-2 border-purple-500' : ''}`}
              ...
              <table className="w-full table-auto border-separate border-spacing-y-3">
                ...
                <tbody>
                  {users.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={8} className="text-center py-4 text-purple-300">No data available.</td>
                    </tr>
                  )}
                </tbody>
              </table>
