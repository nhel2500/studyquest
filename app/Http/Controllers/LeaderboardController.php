    ...
    public function index()
{
    ...
    $allUsers = User::select('name', 'id', 'score', 'level', 'updated_at')
    ->orderBy('name')
    ->get()
    ->map(function ($user) {
        return [
            ...
            'score' => $user->score,
            'level' => $user->level,
            'streak' => null, // or fetch from leaderboard if needed
            'collectibles' => null, // or fetch from leaderboard if needed
    ...
    public function fetchLeaderboard()
{
    $leaderboard = DB::table('leaderboards')
        ...
        ->select(
            ...
            'users.id',
            'leaderboards.score',
            'leaderboards.level_reached as level',
            'leaderboards.max_streak as streak',
            'leaderboards.collectibles_earned as collectibles',
            'leaderboards.updated_at as date'
        )
        ...
        ->map(function ($entry) {
            return [
                ...
                'date' => $entry->date ? \Carbon\Carbon::parse($entry->date)->format('M d, Y') : 'N/A',
            ];
        });
