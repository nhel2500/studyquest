...
export default function Leaderboard({
  ...
  const [normalData, setNormalData] = useState<LeaderboardEntry[]>(normalLeaderboard);
  const [suddenDeathData, setSuddenDeathData] = useState<LeaderboardEntry[]>(suddenDeathLeaderboard);


  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const response = await axios.get('/fetch-leaderboard'); // your route to fetch
        const updatedData = response.data;

        // Update state
        setNormalData(updatedData); // if you only have one mode
        setSuddenDeathData(updatedData); // if you separate sudden death scores later
      } catch (error) {
        console.error('Failed to fetch leaderboard updates', error);
      }
  ...
  function mergeAllUsersWithScores(entries: LeaderboardEntry[], users: LeaderboardEntry[]) {
    ...
    for (const user of users) {
      userMap.set(user.user, {
        user: user.user,
        score: user.score,        // ✅ use the original user score from database!
        level: user.level,
        streak: user.streak,
    ...
    for (const entry of entries) {
      userMap.set(entry.user, entry); // ✅ This still allows updates from the latest leaderboard if needed
    }
...
const mergedNormalLeaderboard = useMemo(() => {
    return mergeAllUsersWithScores(normalData, allUsers);
  }, [normalData, allUsers]); // ✅ Tama, normalData ang nakadepende!

  const mergedSuddenDeathLeaderboard = useMemo(() => {
    return mergeAllUsersWithScores(suddenDeathData, allUsers);
  }, [suddenDeathData, allUsers]); // ✅
      ...
      <div className="mb-6">
        ...
        <div className="bg-card shadow-lg rounded-lg overflow-hidden">
          ...
          <div className="p-4">
            ...
            {activeTab === 'normal' && (
                <>
                    {renderLeaderboardTable(mergedNormalLeaderboard)}
                    {renderMobileLeaderboard(mergedNormalLeaderboard)}
                ...
                <>
                    {renderLeaderboardTable(mergedSuddenDeathLeaderboard)}
                    {renderMobileLeaderboard(mergedSuddenDeathLeaderboard)}
