...
export default function TriviaGame(props: TriviaGameProps) {
  ...
  const handleTimeUp = () => {
    ...
    if (activePowerUp === 'shield') {
      ...
      setSession(prev => {
        ...
        // ✅ Play game over sound if this hit drops hearts to 0
        if (newHearts <= 0) {
          play('gameOver');
        }
  ...
  //automatic save score
  const saveScore = async (finalScore: number) => {

    console.log('Trying to save score:', finalScore);
    try {
      const response = await axios.post('/save-score', { score: finalScore });
  ...
  const handleAnswerSelect = async (answerId: string) => {
    ...
    try {
        ...
        if (isCorrect) {
          ...
          if (collectibleAwarded) {
            ...
            setTimeout(() => {
              ...
              handleNextQuestion();
            }, 3000); // Auto-close after 3 seconds and go to next question
          }
      ...
      if (isCorrect) {
        ...
        if (Math.random() < 0.1) {
          const rarities = ['Common', 'Rare', 'Very Rare', 'Legendary'];
          ...
          const newCollectible = {
            ...
            rarity: randomRarity,
            bonus: randomRarity === 'Legendary' ? '+50 points on next correct answer' : null
          };
  ...
  const handleNextQuestion = () => {
    play('button'); // ✅ Play click sound right away when user clicks "Next"

    // Game over check
    if (session.hearts <= 0) {
      play('gameOver'); // ✅ Optional: play game over if hearts drop to zero here
      saveScore(session.score); // Save score before game over
    ...
    if (rowStatus.row1Completed && rowStatus.row2Completed && rowStatus.row3Completed) {
        saveScore(session.score); // Save score before level complete
  ...
  const handleNextLevel = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const currentRowQuestions = gameData.triviaData.filter(q => q.row === 1);
        if (currentRowQuestions.length > 0) {
          const questionIndex = gameData.triviaData.findIndex(q => q.id === currentRowQuestions[0].id);
          setCurrentQuestion(questionIndex);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setCompletedQuestions(gameData.completedQuestions || []);
          setCollectedItems(gameData.collectedItems || []);
          ...
          // Set AI message
          setAiMessage(Game restarted in ${gameMode} mode! Ready for more Philippine history trivia?);
          setAiMood('happy');
        ...
        toast.dismiss(loadingToast);
        toast.warning('API restart failed. Using client-side restart.');

        // CLIENT-SIDE FALLBACK for restart
        // Reset session
        setSession({
          id: session.id, // Keep the session ID
          level: 1,
          currentRow: 1,
          score: 0,
          streak: 0,
        ...
        // Clear collected items for new game
        setCollectedItems([]);

        // Reset power-ups (optionally give some starter power-ups)
        setAvailablePowerUps(['fifty-fifty']);
